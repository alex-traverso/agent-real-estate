...
- [x] WhatsApp connection, self-serve: onboarding leaves `agencies.whatsapp_phone_number_id` `NULL`, and until this change nothing in the product said so — the panel worked normally while every inbound message was silently dropped by `AgencyService.resolveIdByPhoneNumberId`, and the column could only be set from `seed.ts` or by hand in Supabase. Added: `UpdateAgencyDto` + `AgencyService.updateForAgency` (partial update, explicit `null` clears a column, `23505` on the number or the email mapped to Spanish `ConflictException`s); `PATCH /agencies/me` behind `SupabaseAuthGuard` + `@CurrentAgency()`, which is why `AgencyController` now declares its guards per route instead of on the class; `app/(dashboard)/settings` (agency data + WhatsApp connection with a Conectado/No conectado badge, empty input disconnects); and `WhatsAppBanner`, rendered by `DashboardShell` above every dashboard page from the agency `(dashboard)/layout.tsx` already loads. `AgencyService`'s two in-process caches gained a TTL (`AGENCY_CACHE_TTL_MS`) _and_ explicit eviction on update — without both, an instance that didn't serve the `PATCH` would keep a released number pointing at its former owner and route another tenant's messages to it. **Outbound stays single-tenant:** `WhatsAppService` still reads `META_PHONE_NUMBER_ID`/`META_API_TOKEN` from the env, so Luca always replies from the one configured number; per-agency sending needs tenant-scoped credentials and is out of scope.
...
- [x] Render production deploy verified (`main` branch)
- [x] Vercel production deploy verified (`main` branch)
