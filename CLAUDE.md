...
│                   ├── properties/
│                   ├── leads/
│                   └── settings/     # Agency data + WhatsApp connection
├── packages/
...
### Tables
- `agencies` — root tenant entity. `whatsapp_phone_number_id` is what maps an inbound webhook to a tenant; it is **nullable and empty on a freshly created agency** (Meta's id isn't something an agency has at signup). Until it's set, `AgencyService.resolveIdByPhoneNumberId` finds nothing and every inbound message is dropped — so the agency sets it itself from `/settings` (`PATCH /agencies/me`) and the dashboard shows a "WhatsApp no conectado" banner until it does. `AgencyService`'s in-process caches are TTL'd *and* evicted on update; see `ARCHITECTURE.md` → WhatsApp Connection for why both are required.
- `agency_users` — links Supabase Auth users to their agency; resolves `auth.uid()` → `agency_id` for the admin panel's RLS policies. `UNIQUE (user_id)`: one user belongs to at most one agency (many users can share an agency). A user with no row here has no agency yet — the admin panel sends them to `/onboarding` (`POST /agencies`, behind `SupabaseUserGuard`) to create one, since `SupabaseAuthGuard` requires a linked agency on every other admin route.
